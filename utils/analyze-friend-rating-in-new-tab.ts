...
(function (d) {
  ...
  function insertAnalyzeButton(gameVer: GameVersion, friend: FriendInfo, container: HTMLElement) {
    ...
    if (friend.page === FriendPage.FRIEND_VS) {
      ...
      // Add safety check for parentElement
      container.parentElement?.insertAdjacentElement('afterend', analyzeSpan);
    } else {
      analyzeSpan.className += ' d_b';
      const commentSelector = friend.page === FriendPage.FRIEND_LIST ? '.friend_comment_block' : '.comment_block';
      container.querySelector(commentSelector)?.insertAdjacentElement('afterbegin', analyzeSpan);
    }
  }
      ...
      for (const difficulty of FRIEND_SCORE_URLS.keys()) {
        ...
        scoreList = scoreList.concat(
          await (full ? fetchFriendScoresFull : fetchFriendScores)(
            friend.idx,
            difficulty,
            new SongDatabase(gameVer, undefined, false)
          )
        );
      }
      send('showProgress', '');
  ...
  async function main() {
    ...
    if (
      ...
      const idx = new URLSearchParams(location.search).get('idx') ?? ''; // Fix: fallback to empty string
      if (elem && idx) {
        const info: FriendInfo = {idx, name: getPlayerName(elem) ?? '', grade: '', page: FriendPage.FRIEND_VS};
      ...
      if (elem && idx) { // Fix: Ensure both exist
        const info: FriendInfo = {
          idx,
          name: getPlayerName(elem) ?? '', // Fix: fallback
          grade: getPlayerGrade(elem) ?? '', // Fix: fallback
      ...
      ).map((n) => n.parentElement).filter((n): n is HTMLElement => n !== null); // Fix: filter out null parents
    ...
    window.ratingCalcMsgListener = (
      ...
      if (ALLOWED_ORIGINS.includes(evt.origin)) {
        ...
        if (typeof evt.data !== 'object' || !evt.data.payload) { // Fix: check payload existence
          return;
        }

        const payloadStr = evt.data.payload.toString(); // Fix: ensure payload is string for index use

        if (evt.data.action === 'getFriendRecords') {
          send('gameVersion', gameVer);
          const friend = friends_cache[payloadStr];
          if (friend) {
