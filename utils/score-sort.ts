...
(function (d) {
  ...
  function sortRowsByLevel(rows: NodeListOf<HTMLElement>, reverse: boolean) {
    const map:Map<string, HTMLElement[]> = createMap(CHART_LEVELS, reverse);
    rows.forEach((row) => {
  ...
  function getDxStar(row: HTMLElement) {
    ...
    row.dataset.dxStar = dxStar || 'null';
    return dxStar;
  }
  ...
  async function addSummaryBlock() {
    ...
    function updateRankSummary() {
      ...
      rows.forEach((row) => {
        rankCount[getRankTitle(row) || 'null']++;
      });
    ...
    function updateApFcSummary() {
      ...
      rows.forEach((row) => {
        apfcCount[getApFcStatus(row, true) || 'null']++;
      });
    ...
    function updateSyncSummary() {
      ...
      rows.forEach((row) => {
        syncCount[getSyncStatus(row, true) || 'null']++;
      });
    ...
    function updateDxStarSummary() {
      ...
      rows.forEach((row) => {
        dxStarCount[getDxStar(row) || 'null']++;
      });
