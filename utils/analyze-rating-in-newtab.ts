...
(function () {
  ...
  async function fetchSelfRecords(
    ...
    // Fetch player grade
    const playerGrade = isOnFriendPage ? undefined : getPlayerGrade(document.body);
    if (playerGrade) {
      send('playerGrade', playerGrade);
    }
    const songDb = new SongDatabase(gameVer, undefined, false);
    // Fetch recent plays
    const visitedCharts = new Set<string>();
    send('showProgress', statusText(LANG, Difficulty.UTAGE, false));
  ...
  function insertAnalyzeButton(gameVer: GameVersion, playerName: string) {
    ...
    if (location.pathname.indexOf('/maimai-mobile/playerData/') >= 0) {
      ...
      if (playCommentDiv) {
        ...
        const nameBlock = profileBlock.querySelector('.name_block');
        if (nameBlock?.parentElement) {
          nameBlock.parentElement.append(analyzeSpan);
        }
      }
    }
  }
  ...
  async function main() {
    ...
    const playerName = isOnFriendPage ? '' : (getPlayerName(document.body) ?? '');
