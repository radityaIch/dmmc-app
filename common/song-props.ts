...
export class SongDatabase {
  constructor(
    readonly gameVer: GameVersion = LATEST_VERSION,
    readonly region: GameRegion = GameRegion.Intl,
  ...
  updateSong(update: Partial<SongProperties>): boolean {
    ...
    if (update.name) {
      if (map.has(update.name)) {
        key = update.name;
      } else if (update.genre) {
        key = getSongNickname(update.name, update.genre);
      } else {
        key = update.name;
      }
    }
