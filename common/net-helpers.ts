...
export async function fetchGameVersion(dom: Document | HTMLElement): Promise<GameVersion> {
  const cacheExpirationStr = window.localStorage.getItem(EXPIRATION_KEY);
  const cacheExpiration = cacheExpirationStr !== null ? parseInt(cacheExpirationStr) : NaN;
  if (!isNaN(cacheExpiration) && cacheExpiration >= Date.now()) {
    const cachedGameVerStr = window.localStorage.getItem(CACHE_KEY);
    if (cachedGameVerStr !== null) {
      const cachedGameVer = parseInt(cachedGameVerStr);
