...
export function getSongName(row: HTMLElement): string {
  ...
  if (playRecordSongNameElem) {
    ...
    return (
      playRecordSongNameElem.childNodes.item(playRecordSongNameElem.childNodes.length - 1)
        ?.nodeValue ??
      ''
    );
  }

  const musicNameElem = row.querySelector<HTMLElement>('.music_name_block');
  return normalizeSongName(musicNameElem?.innerText ?? '');
}
...
export function getChartDifficulty(row: HTMLElement) {
  ...
  const match = row.className.match(/music_([a-z]+)_score_back/);
  const d = match?.[1] ?? '';
